...
public class PurchaseOrders_Method extends BaseTest{
		...
		public void createPurchaseOrder() throws Exception {
			try
			{
			    ...
			    String tempName = testData.get("PruchaseOrderName")+Common.getUniqueID(5);
		...
		//	AC.clickItemSubtypeCancel();
			...
			if(driver.findElement(By.xpath("//div[contains(@class,'sweet-alert') and contains(@class,'visible')]")).getAttribute("innerHTML").contains("Success Message"))
			{
				...
				Common.logTrace("ItemSKUName --> "+testData1.get("ItemSKUName")+" created Successfully");
			}
			else
			{
				Common.fail(driver,"ItemSKUName--> "+testData1.get("ItemSKUName")+" not created Successfully");
				Common.logTrace("ItemSKUName--> "+testData1.get("ItemSKUName")+" not created Successfully");
			}
			try{
			...
			if(driver.findElement(By.xpath("//div[contains(@class,'sweet-alert') and contains(@class,'visible')]")).getAttribute("innerHTML").contains("Success Message"))
			{
				...
				Common.logTrace("ContractName --> "+testData1.get("ContractName")+" created Successfully");
			}
			else
			{
				Common.fail(driver,"ContractName--> "+testData1.get("ContractName")+" not created Successfully");
				Common.logTrace("ContractName--> "+testData1.get("ContractName")+" not created Successfully");
			}
			try{
